...
module.exports = {
  ...
  execute: async (client, message, args, data, db) => {
    ...
    message.channel.send(`Succesfully bought ${amount} coins for your server, you can see the currents order with \`-order\`|\`-info\`.`)

    await new Promise(resolve => setTimeout(resolve, 100))

    await db.set(`code_${message.guild.id}`, link)

    await data.logs.unshift(`[-${amount}] - Advertise ${message.guild.name} server.`)

    await db.set(`logs_${message.author.id}`, data.logs)

    await db.set(`description_${message.guild.id}`, `${description === undefined ? "" : description}\nhttps://discord.gg/${link}`)

    await db.add(`orders_${message.guild.id}`, amount)

    await db.subtract(`coins_${message.author.id}`, amount)
  }
} 